...
cliproxy is an example usage of etcd's [raft library](../../raft). It provides a simple REST API for a key-value store cluster backed by the [Raft][raft] consensus algorithm.
...
cd <directory>/src/go.etcd.io/etcd/contrib/cliproxy
go build -o cliproxy
```

### Running single node cliproxy

First start a single-member cluster of cliproxy:

```sh
cliproxy --id 1 --cluster http://127.0.0.1:12379 --port 12380
```

Each cliproxy process maintains a single raft instance and a key-value server.
...
This will bring up three cliproxy instances.
...
```sh
cliproxy --id 1 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 12380
cliproxy --id 2 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 22380
cliproxy --id 3 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 32380
```
...
```sh
cliproxy --id 4 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379,http://127.0.0.1:42379 --port 42380 --join
```
...
The cliproxy consists of three components: a raft-backed key-value store, a REST API server, and a raft consensus server based on etcd's raft implementation.
...
For cliproxy, this commit channel is consumed by the key-value store.
