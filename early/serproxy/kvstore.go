...
func NewKVStore(snapshotter *snap.Snapshotter, proposeC chan<- string, commitCTemp <-chan *string, commitC chan *string, errorC <-chan error, Id int) *Kvstore {
	...
	var Lmap sync.Map
	s := &Kvstore{proposeC: proposeC, kvStore: make(map[string]string), snapshotter: snapshotter, id: Id, DB: redisCli, LatencyMap: Lmap}

	go func() {
		for data := range commitCTemp {
			commitC <- data
		}
	}()
...
func (s *Kvstore) Propose(k string, v string, rw string, idkv string) {
...
func (s *Kvstore) readCommits(commitC <-chan *string, errorC <-chan error) {
	...
	for data := range commitC {
		...
		if err := dec.Decode(&dataKv); err != nil {
			log.Fatalf("serproxy: could not decode message (%v)", err)
		}
